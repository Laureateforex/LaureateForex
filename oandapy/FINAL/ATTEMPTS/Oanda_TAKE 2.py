...
def DataFrameFactory(r, colmap=None, conv=None):
    ...
    """"LFX: x given by us, Splits the string in the Series/Index from the beginning, at the specified delimiter string. Equivalent to str.split()."""
    ...
    dfH  = pd.DataFrame([list(record_converter(rec, cmap)) for rec in s.get('candles')])
...
if __name__ == "__main__":
    ...
    for i in instruments:
        ...
        # Do something with the dataframes Hourly
    ...
    print(df['EUR_USD'].iloc[1])
    print(df['EUR_GBP'].iloc[1])
    print(df['AUD_USD'].iloc[1])
    #
